...
// In-portal inbox view: lists messages delivered to the current user; mark read
// / dismiss. Lives in a component (not a page file) so both the /inbox route and
// the Communications landing page can render it without exporting a non-default
// symbol from a route/page module (a Next.js constraint).
      ...
      {showHeader ? (
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 ? (
              <Badge className="bg-primary/10 text-primary hover:bg-primary/10">{unreadCount} unread</Badge>
            ) : null}
    ...
    <li className={unread ? 'bg-primary/[0.03]' : undefined}>
      <button type="button" onClick={onToggle} aria-expanded={open}
        ...
        <span className="min-w-0 flex-1">
          ...
          {!open && preview ? <span className="mt-0.5 line-clamp-1 text-sm text-muted-foreground">{preview}</span> : null}
        </span>
      ...
      {open ? (
        <div className="border-t bg-muted/20 px-4 py-4">
          <div
            ...
            // (@/lib/communications/markdown escapes input, emits only safe tags).
            dangerouslySetInnerHTML={{ __html: item.body_html }}
          />
          <div className="mt-4 flex justify-end">
            <Button variant="outline" size="sm" onClick={onDismiss} disabled={busy}>
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />} Dismiss
            </Button>
          </div>
        </div>
      ) : null}
...
/** Strip tags + collapse whitespace for a one-line preview. */
